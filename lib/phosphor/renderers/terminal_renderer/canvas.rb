      ...
      class Canvas
        ...
        def print_at(x_pos, y_pos, string, owner = nil, color_pair: nil, foreground_color: nil, background_color: nil)
          @mutex.synchronize do
            ...
            ansi = if color_pair
                     color_pair.ansi_sequence
                   elsif foreground_color || background_color
                     ColorPair.new(foreground_color, background_color).ansi_sequence
                   end
        ...
        def render
          @mutex.synchronize do
            ...
            @was_changed = false
            out = String.new(capacity: 4096, encoding: "UTF-8")
        ...
        def append_batch(out, x, y, batch, ansi)
          out << "\e[#{y + 1};#{x + 1}H"
          if ansi
            out << ansi
            out << batch
        ...
        def build_batch(x, pixels_row, cp_row, prev_pixels_row, prev_cp_row)
          cp = cp_row[x]
          batch = String.new(capacity: 32, encoding: "UTF-8")
          batch << pixels_row[x]
          x += 1
          ...
          while x < @width
            next_cp = cp_row[x]
            break if next_cp != cp
            break if pixels_row[x] == prev_pixels_row[x] && next_cp == prev_cp_row[x]
